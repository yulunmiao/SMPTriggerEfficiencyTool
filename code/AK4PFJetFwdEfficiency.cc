int AK4PFJetFwdEfficiency(){
    //TFile *f = new TFile("../hist/JetTrigger2024BCDEF.root");
    TFile *f = new TFile("../hist/JetTrigger2024G.root");

    TString triggername[3]={"HLT_PFJetFwd400","HLT_PFJetFwd450","HLT_PFJetFwd500"};
    ...
    for(unsigned int i=0;i<3;i++){
        ...
        eff_pt[i]->SetTitle(";leading jet p_{T}(GeV);efficiency");
        ...
        eff_eta[i]->SetTitle(";leading jet #eta;efficiency");
    ...
    eff_pt[2]->Draw("same");
    l->Draw();
    c->SaveAs("Run2024G_PFjetfwd_pt.png");
