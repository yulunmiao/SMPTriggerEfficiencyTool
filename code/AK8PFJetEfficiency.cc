int AK8PFJetEfficiency(){
    //TFile *f = new TFile("../hist/JetTrigger2024BCDEF.root");
    TFile *f = new TFile("../hist/JetTrigger2024G.root");

    TString triggername[2]={"HLT_AK8PFJet500","HLT_AK8PFJet550"};
    
    Color_t color[2]={kBlack,kRed};
    TH1D* hpass_pt[2]; 
    TH1D* hinclusive_pt[2];
    TH1D* hpass_eta[2]; 
    TH1D* hinclusive_eta[2];
    TEfficiency* eff_eta[2];
    ...
    eff_pt[1]->Draw("same");
    l->Draw();
    c->SaveAs("Run2024G_AK8PFjet_pt.png");
    ...
    eff_eta[1]->Draw("same");
    l1->Draw();
    c1->SaveAs("Run2024G_AK8PFjet_eta.png");
    return 0;
}
